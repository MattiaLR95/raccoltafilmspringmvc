...
public class FilmController {
	...
	public String deleteFilm(@PathVariable(required = true) Long idFilm, Model model) {
		FilmDTO filmDaEliminare = FilmDTO.buildFilmDTOFromModel(filmService.caricaSingoloElemento(idFilm),false);
		model.addAttribute("delete_film_attr", filmDaEliminare);
		return "film/delete";
	}
	
	@PostMapping("/saveDelete/{idFilm}")
	public String saveDelete(@PathVariable(required = true) Long idFilm, Model model, RedirectAttributes redirectAttrs) {
		filmService.rimuovi(idFilm);
		redirectAttrs.addFlashAttribute("successMessage", "Operazione eseguita correttamente");
		return "redirect:/film";
	}
	
	@GetMapping("/update/{idFilm}")
	...
	@PostMapping("/edit")
	public String updateRegista(@Valid @ModelAttribute("edit_film_attr") FilmDTO filmDTO, BindingResult result,
			RedirectAttributes redirectAttrs, HttpServletRequest request) {
